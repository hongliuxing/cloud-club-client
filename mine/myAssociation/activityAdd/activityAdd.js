...
Page({
    ...
    onSubmit() {
        ...
        if (this.data.title == "") {
            return app.globalData.toast("请输入活动标题?")
        }
        if (this.data.title.length > 32) {
            return app.globalData.toast("标题太长!")
        }
        if (this.data.start_date == "年/月/日") {
            return app.globalData.toast("请选择开始时间的日期?")
        }
        if (this.data.start_time == "时:分") {
            return app.globalData.toast("请选择开始时间的时分?")
        }
        if (this.data.end_date == "年/月/日") {
            return app.globalData.toast("请选择结束时间的日期?")
        }
        if (this.data.end_time == "时:分") {
            return app.globalData.toast("请选择结束时间的时分?")
        }
        ...
        if (new Date(brief_start).getTime() > new Date(brief_end).getTime()) {
            return app.globalData.toast("开始时间不能大于结束时间?")
        }
        if (this.data.content == "") {
            return app.globalData.toast("请输入活动内容呢?")
        }
        if (this.data.content.length > 1000) {
            return app.globalData.toast("活动内容太长[ " + this.data.content.length + "/1000 ]!")
        }
        if (this.data.imageList.length == 0) {
            return app.globalData.toast("请上传活动图片?")
        }
        let data = {
        ...
        if (that.data.activity_id) {
            ...
            let array = [];
            array = that.data.imageList.filter((item) => {
                return that.data.oldImageList.indexOf(item) === -1
            })
            data["imgs"] = array
        }
                ...
                uploadUtilActivity(URLs.UPLOAD_CLUB_ACTIVITY_PIC, res.tempFilePaths[0]).then(result => {
                    ...
                    Promise.all(arr)
                        .then(resultList => {
                            ...
                            wx.hideLoading()
                            app.globalData.toast("上传失败，请重新上传")
                        });
